...
def test_bat_traffic_overview(traffic_page):
    ...
    2. Verify date picker is visible
    ...
    assert traffic_page.date_picker.inner_text() == "Last 30 Days", 'Wrong default time'
    assert traffic_page.traffic_metric_selector.is_visible(), 'drop down is not visible'
    assert traffic_page.traffic_requests_grid_summary.inner_text()[0:8] == "Requests", 'Wrong summary'
    assert traffic_page.traffic_errors_grid_summary.inner_text()[0:6] == "Errors", 'Wrong summary'
